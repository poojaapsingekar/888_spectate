...
from db_models import create_event,get_events_with_name_like
...
class Event(Resource):
    ...
    def post(self):
        ...
        if '<SPORT>' in sport['name'] or '</SPORT>' in sport['name']:
            return {'error_message': '<SPORT> or </SPORT> in the sport name is not allowed'}, 400
        sport_name=f'<SPORT>{sport["name"]}</SPORT>'
    ...
    def get(self):
        if request.args.get('name_like') is not None:
            ...
            return get_events_with_name_like(pattern)
        print (request.args.get('name_like'), request.args.get('num_active_selections'), request.args.get('from_datetime'), request.args.get('to_datetime'))
